...
export const getVehicles = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    const vehicles = await vehicleService.getVehiclesByCompanyId(companyId, skip, limit);
...
export const getVehicle = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const companyId = req.user.companyId;
    const vehicle = await vehicleService.getVehicleByIdAndCompanyId(req.params.id, companyId);
...
export const updateVehicle = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    const updatedVehicle = await vehicleService.updateVehicle(req.params.id, companyId, value);
...
export const deleteVehicle = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const companyId = req.user.companyId;
    await vehicleService.deleteVehicle(req.params.id, companyId);
